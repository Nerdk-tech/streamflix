...
// --- API CONFIGURATION ---
// Confirmed API Parameters:
const API_KEY = "Godszeal"; 
const BASE_URL = "https://gzmovieboxapi.vercel.app/api/";

// API ENDPOINT URLs
const HOT_CONTENT_URL = `${BASE_URL}hot-movies-series?apikey=${API_KEY}`; // Used for homepage
const SEARCH_URL = `${BASE_URL}search?apikey=${API_KEY}`;                 // Used for search bar
const MEDIA_URL = `${BASE_URL}media?apikey=${API_KEY}`;                   // Used for streaming
const DETAIL_URL = `${BASE_URL}item-details?apikey=${API_KEY}`;           // Used for movie info page
...
// Utility to manage which sections are visible (Home, Search, or Detail)
function toggleContentSections(isSearching, isDetail) {
    ...
    document.getElementById('video-player-section').style.display = 'none'; // Always hide player on page switch
}

// Clears the search and returns to the default homepage view
function clearSearch() {
    document.getElementById('search-input').value = "";
    toggleContentSections(false, false); // Not searching, not viewing detail (show home)
    document.querySelector('.main-container').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

// Creates the HTML card structure for a movie/series item
...
// ====================================================================
// API 1: Fetch and Display Hot Movies/Series (Homepage) - WITH ERROR HANDLING
// ====================================================================
async function fetchHotContent() {
    ...
    try {
        ...
        if (!response.ok) {
            // Check for HTTP errors (like 404, 500, or 403)
        ...
        const errorMessage = `Failed to load content. Error: ${error.message}. This is likely a network (CORS) or temporary API issue.`;
...
async function fetchSearchResults() {
    ...
    toggleContentSections(true, false); // Show search, hide detail/home
...
async function showDetailPage(subjectId, detailPath, title, posterUrl) {
    ...
    toggleContentSections(false, true); // Hide home/search, show detail page
        ...
        if (data.status === 'success' && data.data) {
            ...
            detailPage.innerHTML = `
                <div class="detail-header">
                    <img src="${posterUrl}" alt="${item.title}" class="detail-poster">
                    <div class="detail-info">
                        ...
                        <button class="detail-play-button" onclick="fetchAndDisplayStream('${subjectId}', '${detailPath}', '${item.title}')">
...
// ====================================================================
// API 4: Fetch and Display the Stream - FIXED FOR ROBUSTNESS
// ====================================================================
async function fetchAndDisplayStream(subjectId, detailPath, title) {
    ...
    try {
        ...
        // **FIXED: Checking for the necessary data fields (data.data and data.data.resource) 
        // directly, which prevents the 'Unknown response structure' error if the 'status' field is missing or malformed.**
        if (data && data.data && data.data.resource) {
            ...
            if (videoLink) {
                ...
                // The .play() catch block is for handling browser autoplay restrictions/CORS stream issues
        ...
        // Check for an explicit error message from the API
        ...
        // Fallback for completely unexpected/malformed responses
        } else {
